document.addEventListener("DOMContentLoaded", function () {
    ...
    async function fetchList() {
        ...
        try {
            ...
            console.error(e);
            tbody.innerHTML = `<tr><td colspan="7" style="color:var(--accent);text-align:center;">Failed to connect</td></tr>`;
        } finally {
    ...
    function renderRows(data) {
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:var(--text-muted)">No transfers found.</td></tr>`;
            return;
        }
        ...
        data.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td><span style="color:var(--text-muted); font-size: 0.85em;">${item.source_warehouse_name || "-"}</span></td>
                <td><span style="color:var(--text-muted); font-size: 0.85em;">${item.destination_warehouse_name || "-"}</span></td>
                <td>${dateStr}</td>
        ...
        document.querySelectorAll(".view-btn").forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                window.location.href = `transfer-form.html?id=${id}`;
            });
        });
    }
            ...
            columnData.forEach(item => {
                ...
                card.innerHTML = `
                    ...
                    <div class="kanban-card-subtitle" style="font-size: 0.75rem; color: var(--text-muted);">
                        ${item.source_warehouse_name || "-"} <br/>➔ ${item.destination_warehouse_name || "-"}
                    </div>
                ...
                card.addEventListener("click", () => {
                    window.location.href = `transfer-form.html?id=${item.id}`;
                });
    ...
    if (newOpBtn) {
        newOpBtn.addEventListener("click", () => {
            window.location.href = `transfer-form.html`;
        });
    }
